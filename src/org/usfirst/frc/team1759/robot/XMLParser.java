 ...
 *
 *         This is a class specifically intended to parse XML strings that look
 *         like this (though not necessarily in this node order):
 * 
 *         <pre>
 * {@code
 * <message>
 ...
 *         </pre>
 * 
 *         Yeah, we could probably write some XSD to do this validation for us,
 *         but checking the XML programmatically took less time to write.
 */
...
public class XMLParser extends DefaultHandler {
	...
	private enum Location {
		...
		/**
		 * We're inside <code>{@code <message>}</code>, but not inside
		 * <code>{@code <data>}</code>.
		 */
		TOPLEVEL,
	...
	/**
	 * Keeps track of where the SAX parser is so we can validate element
	 * positions.
	 */
	...
	/**
	 * An error during parsing sets this flag, which forces the rest of the
	 * parsing to be subsequently skipped.
	 */
	private boolean parseError = false;

	/**
	 * The list of XML elements that a valid PapasVision XML document will
	 * contain.
	 */
	final static List<String> recognizedElementNames = Arrays.asList("message", "type", "timestamp", "data",
			"papasdistance", "papasangle", "solutionfound", "solutiontype");

	/**
	 * This is a checklist--any complete XML document will see all of the
	 * elements that we officially recognize at least once.
	 */
	 ...
	 *            The input XML document (as a string, for now.) Typically this
	 *            comes from a camera over the network.
	 * @return The PapasVision parameters we have extracted from the input
	 *         document, as a {@link PapasData}.
	 */
	public PapasData parse(String xmlDocumentString) throws Exception {
		...
		try {
			...
			for (String elementName : countsForRecognizedElements.keySet()) {
				...
				if (count != 1) {
					throw new XMLParserException(xmlDocumentString, String.format(
							"Expected to see exactly one <%s> node in the XML document, not %d.", elementName, count));
				}
			}
			...
			String message = String.format(
					"Caught a SAXException when parsing incoming PapasVision XML document.  The message was: \"%s\".  The XML document was: %s",
					e.getMessage(), xmlDocumentString);
	...
	@Override
	/**
	 * Whenever our parser encounters the opening of any XML node in the string
	 * we're parsing, this method gets called.
	 *
	 * For our part, all we do is set a variable, currentElement, to the last
	 * node we saw.
	 */
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		...
		if (recognizedElementNames.contains(qNameInLowerCase)) {
			...
			// This is a recognized element. Count the number of these we have
			// seen so far.
			//
	...
	@Override
	/**
	 * This event handler gets called once for the end of every XML element in
	 * the SAX stream.
	 */
	...
	@Override
	/**
	 * This is called when we read the text contents between the start and end
	 * tags of any XML node.
	 *
	 * Based on what our current element was, this tells us what data we need to
	 * capture.
	 *
	 * For instance, if our XML is "<code>{@code <Foo>Bar</Foo>}</code>", then
	 * ch[] will be { 'B', 'a', 'r' }, start will be 0, and length will be 3.
	 *
	...
	public void characters(char ch[], int start, int length) throws SAXException {
		...
		case "papasdistance":
			...
			if (papasData.papasDistanceInInches < 0) {
				// Not a valid message; this would imply that the camera is
				// behind the target!
				errorMessage = String.format("The camera is NOT %.3f inches behind the target, you liar.",
						-papasData.papasDistanceInInches);
				parseError = true;
				return;
			}
			break;
		...
		case "solutiontype":
			...
			if (!content.equalsIgnoreCase("peg") && !content.equalsIgnoreCase("boiler")) {

				errorMessage = String.format(
						"Unrecognized <SolutionType> \"%s\".  Only \"Peg\" and \"Boiler\" are accepted (case-insensitive.)",
						content);
				parseError = true;
				return;
			}
		...
		default:
			...
			// We don't expect to read the contents of anything which is not a
			// recognized element.
			// Those were rejected early. Control should never make it here.
			errorMessage = String.format(
					"Internal error: unrecognized element <%s> somehow escaped the startElement() filter.",
					currentElement);
			parseError = true;
			return;
		}
