...
public class TerminalTest {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 0) {
			...
			System.err.println("[error] Missing sub-command; must be either, 'parse', 'listen', or 'fit'.");
			usage();
			...
			if (subCommand.equals("parse")) {
				...
				if (args.length == 1) {
					...
					for (int i = 1; i < args.length; i++) {
						// System.out.println(String.format("[debug] XML
						// document #%d: \"%s\"",
						// i, args[i]));
				...
				if (args.length > 3) {
					...
					thread.start();
					System.out.printf(
							"[main] Spawned the ServerRunnable thread (id = %d) to listen on localhost:%d for %.2f seconds...\n",
					...
					while (System.currentTimeMillis() - startTime < timeout) {
						...
						if (runnable.getPapasData() != null) {
							...
							// Only print something if the situation has
							// changed.
							if (lastKnownTwist != currentTwist) {
								if (currentTwist == 0) {
									System.out.printf(
											"  * AutoAimCommandImpl reports that it has reached its target (isFinished() == true).\n");
								} else {
									System.out.printf(
											"  * AutoAimCommandImpl reports that it has rotated %s (twist == %f).\n",
											(currentTwist > 0 ? "counterclockwise" : "clockwise"), currentTwist);
								}
							}
						}
				...
				System.out.println(curveFitter.getPlottingCommand(
						String.format("Curve fitting: '%s' (%s)", algorithmName, curveFitter.toString())));
	...
	public static void usage() {
		...
		System.out.printf("Usage: %s parse XML-STRING\n\n"
				...
				DEFAULT_TIMEOUT_MILLISECONDS, ServerRunnable.DEFAULT_PORT, programName, algorithmsList.toString(),
				programName);
	}
}
