 ...
 * @author uakotaobi
 *
 *         This class tries to calculate a line of best fit for the given set of
 *         data points.
 *
 ...
 *         https://en.wikipedia.org/wiki/Simple_linear_regression. The line of
 *         regression is supposed to pass through the center of mass. (The graph
...
public class LinearRegressionCurveFitter extends AbstractCurveFitter {
	...
	protected List<Double> getCoefficients(List<Coordinate> dataPoints) {
		...
		double denominator = Math.sqrt((ddSum - (dSum * dSum) / n) * (vvSum - (vSum * vSum) / n));
		...
		// That's it -- y = mx + b, so F(x) = mx + b, which really means that
		// F(d) = m*d + b.
