 ...
 *
 *         Curve fitters take a set of data points in (Distance, Velocity) space
 *         (where 0 < Distance and 0 <= Velocity <= 1) and calculate the
 *         coefficients of polynomial that best fits those data points. The
 *         polynomial can subsequently be used to predict the proper velocity
 *         for a given distance, or simply plotted on a graph.
 */
...
public abstract class AbstractCurveFitter {
	...
	protected class Coordinate {
		public Coordinate(double d, double v) {
			this.d = d;
			this.v = v;
		}

		/**
		 * The distance (from the camera target) this coordinate represents, in
		 * inches.
		 */
		public double d;

		/**
		 * The shooting velocity this coordinate represents, ranging from 0 for
		 * minimum power to 1 for maximum power.
		 */
		public double v;
	}
	 ...
	 *
	 * @param dataPoints
	 *            A list of length 2*N, where N is the number of data points to
	 *            analyze. The even indices of the array, starting with element
	 *            0, and the distance coordinates. The odd indices of the array,
	 *            starting with 1, are velocity coordinates.
	 *
	 *            If the length of the array is an odd number, the last element
	 *            will be ignored.
	 */
	...
	/**
	 * Gets the coefficients of the polynomial curve that best fits the given
	 * data.
	 */
	...
	/**
	 * Gets the GNUPlot command to graph both the set of data points and the
	 * curve that best fits them.
	 *
	 * @param title
	 *            The title of the graph.
	 *
	 * @return A string which can be executed to create a graph image called
	 *         "result.png" in the current directory. You will require your
	 *         local system's gnuplot package.
	 */
	public String getPlottingCommand(String title) {

		// To keep the graph from looking erroneously crazy, restrict it to the
		// natural
		...
		// Unfortunately, to enter inline data for the plot, we have to use
		// newlines.
	...
	/**
	 * Evaluates the polynomial at the given distance coordinate and returns the
	 * corresponding velocity. Calls {@link getCoefficients} to do its dirty
	 * work -- after that, it's just multiplying and adding.
	 *
	 * @param d
	 ...
	 *
	 * For instance, if {@link getCoefficients} returns {2, -3, 1}, then this
	 * function will return "v = 2.00 * d**2 + -3.00 * d + 1.00".
	 */
	...
	/**
	 * Gets the coefficients of the polynomial curve v = F(d) that best fits the
	 * given data.
	 *
	 ...
	 * @return The coefficients of the polynomial, starting with the coefficient
	 *         for the term with the highest degree.
	 */
	protected abstract List<Double> getCoefficients(List<Coordinate> dataPoints);

	/**
	 * A optional override that can be used to report intermediate variables
	 * calculated during the last {@link getCoefficients} call.
	 * 
	 * @return A {@link Map<String, String>} mapping variable names to variable
	 *         contents. The base class returns an empty map.
	 */
